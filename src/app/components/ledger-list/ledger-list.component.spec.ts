import { TestBed } from "@angular/core/testing";

import { of } from "rxjs";

import { LedgerListComponent } from "./ledger-list.component";
import { Ledger } from "../../interfaces/ledger";
import { LedgerService } from "../../services/ledger.service";

describe('LedgerListComponent', () => {
  let component: LedgerListComponent;
  let ledgerServiceSpy: jasmine.SpyObj<LedgerService>;

  beforeEach(() => {
    ledgerServiceSpy = jasmine.createSpyObj('LedgerService', ['getLedgers', 'archiveLedger']);

    TestBed.configureTestingModule({
      imports: [LedgerListComponent],
      providers: [
        { provide: LedgerService, useValue: ledgerServiceSpy },
      ]
    });

    component = TestBed.createComponent(LedgerListComponent).componentInstance;
  });

  it ('should create the component', () => {
    expect(component).toBeTruthy();
  });

  describe('ngOnInit', () => {
    it ('should get all active ledgers', () => {
      const ledgers: Ledger[] = [
        { id: 'ledger1', name: 'Ledger 1', description: 'Description 1', 'archived': false, ownerId: 'owner1' },
        { id: 'ledger2', name: 'Ledger 2', description: 'Description 2', 'archived': false, ownerId: 'owner1' }
      ];

      ledgerServiceSpy.getLedgers.and.returnValue(of(ledgers));

      component.ngOnInit();

      expect(ledgerServiceSpy.getLedgers).toHaveBeenCalled();

      const subscription = component.ledgers?.subscribe((ledgers) => {
        expect(ledgers.length).toBe(2);
        expect(ledgers[0].name).toBe('Ledger 1');
        expect(ledgers[1].name).toBe('Ledger 2');
      });

      subscription?.unsubscribe();
    });
  });

  describe('selectLedger', () => {
    it ('should select a ledger', () => {
      const ledger: Ledger = { id: 'ledger1', name: 'Ledger 1', description: 'Description 1', archived: false, ownerId: 'owner1' };

      component.selectLedger(ledger);

      expect(component.selectedLedger).toEqual(ledger);
    });
  });

  describe('archiveLedger', () => {
    it ('should archive a ledger', () => {
      const ledgerId = 'ledger1';

      component.archiveLedger(ledgerId);

      expect(ledgerServiceSpy.archiveLedger).toHaveBeenCalledWith(ledgerId);
    });
  });
});
